...
def find_two_laser_point_backmarker(rectangle, frame):
    v1, v2, v3, v4 = rectangle
    line_a, line_b, line_c = find_laser_line_backmarker(rectangle, frame)
    upper_line = find_line_equation(v1[0][0], v1[0][1], v2[0][0], v2[0][1])
    lower_line = find_line_equation(v3[0][0], v3[0][1], v4[0][0], v4[0][1])
...
def find_laser_point_platemarker(ellipse, frame: np.ndarray):
    ...
    # Pick the first non-zero value in the mask
    point = cv2.findNonZero(laser_mask)
    if point is None:
        raise Exception("Point not detected")

    laser_point = point[0][0]
    # Convert to non-cropped coordinates
    laser_point = [
        round(laser_point[0] + center_x - 150),
        round(laser_point[1] + center_y + 100),
    ]
    return laser_point


def find_all_laser_points_obj(ellipse, frame: np.ndarray):
